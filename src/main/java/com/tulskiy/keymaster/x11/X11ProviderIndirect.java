 ...
 * Date: 6/13/11
 */
public class X11ProviderIndirect extends Provider {
...
//                Lib.XSetErrorHandler(errorHandler);
    ...
    private void register(X11HotKey hotKey) {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            Lib.XGrabKey(display, code, flags, window, true, GrabModeAsync, GrabModeAsync);
        }
    }
    ...
    private void registerMedia(X11HotKey hotKey) {
        ...
        hotKey.modifiers = 0;
        hotKey.code = keyCode;
        Lib.XGrabKey(display, keyCode, 0, window, true, GrabModeAsync, GrabModeAsync);
    }
